...
const Header: React.FC = () => {
  ...
  const currentTitle = navMain.find((item) => item.url === location.pathname)?.title || '首页'
  ...
  return (
    <header className="flex h-16 shrink-0 items-center p-4 justify-between gap-2 transition-[width,height] ease-linear">
      ...
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" className="relative size-7 cursor-pointer rounded-full">
          <Bell />
