...
const _UDSService = class _UDSService extends EventEmitter {
  ...
  sendServerResult(resultType, content, options = {}) {
    const message = {
...
const _NativeProcessManager = class _NativeProcessManager {
  ...
  getNativeAppPath() {
    if (!app.isPackaged) {
      return path__default.join(app.getAppPath(), "assets/MiaoyanSwift.app/Contents/MacOS/MiaoyanSwift");
    }
    let appPath;
    if (process.platform === "darwin") {
      appPath = path__default.join(
        process.resourcesPath.replace("/Resources", ""),
        "Resources",
        "Helpers/MiaoyanSwift.app/Contents/MacOS/MiaoyanSwift"
      );
    } else {
      appPath = path__default.join(process.resourcesPath, "assets", "MiaoyanSwift");
    }
    return appPath;
  }
};
...
const _MessageBus = class _MessageBus {
  constructor() {
...
__publicField(_MessageBus, "instance", null);
let MessageBus = _MessageBus;
...
function createWindow() {
  ...
  if (VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(path$1.join(RENDERER_DIST, "index.html")).then();
  }
}
...
app.whenReady().then(async () => {
  app.setAboutPanelOptions({
    ...
    iconPath: path$1.join(__dirname$1, "../../assets/icon.icns")
  });
  createWindow();
});
export {
