...
import require$$0$1, { spawn } from "child_process";
...
class UDSService extends EventEmitter {
  ...
  handleClientConnection(socket) {
    ...
    socket.on("data", (data) => {
      clientInfo.lastActivity = /* @__PURE__ */ new Date();
   ...
   * 处理录音相关消息
   */
  handleMessageRecording(clientId, message) {
  ...
  sendToClient(clientId, message) {
    ...
    try {
      ...
      log.error(`Error sending message to client ${clientId}:`, error2);
      this.clients.delete(clientId);
      return false;
    }
  }
  sendServerResult(resultType, content2, options = {}) {
    const message = {
    ...
    console.log(`Sending server result: ${resultType} - ${content2}`);
...
class NativeProcessManager {
  ...
  async start() {
    ...
    try {
      ...
      if (!fs__default.existsSync(appPath)) {
        const err = `MiaoyanSwift 文件不存在: ${appPath}`;
        log.error(err);
        throw new Error(err);
      }
      try {
        fs__default.chmodSync(appPath, "755");
      } catch (chmodError) {
      ...
      childProcess.on("error", (error2) => {
        console.error("[MiaoyanSwiftManager] Process error:", error2);
        this.nativeProcess = null;
      });
      childProcess.on("exit", (code2, signal) => {
        console.log(`[MiaoyanSwiftManager] Process exited with code: ${code2}, signal: ${signal}`);
        this.nativeProcess = null;
      });
  ...
  getNativeAppPath() {
    if (!app$1.isPackaged) {
      return require$$2$1.join(app$1.getAppPath(), "assets/MiaoyanSwift.app/Contents/MacOS/MiaoyanSwift");
    }
    let appPath;
    if (process.platform === "darwin") {
      appPath = require$$2$1.join(
        process.resourcesPath.replace("/Resources", ""),
        "Resources",
        "Helpers/MiaoyanSwift.app/Contents/MacOS/MiaoyanSwift"
      );
    } else {
      appPath = require$$2$1.join(process.resourcesPath, "assets", "MiaoyanSwift");
    }
    return appPath;
  }
}
...
class ProcessManager {
  ...
  async setupIPCMainHandlers() {
    ...
    ipcMain$1.handle(IPC_SHOW_STATUS_WINDOW_CHANNEL, () => {
    ...
    ipcMain$1.handle(IPC_HOT_KEY_SETTING_START_CHANNEL, (_, mode) => {
...
app$1.commandLine.appendSwitch("--disable-web-security");
app$1.commandLine.appendSwitch("--allow-running-insecure-content");
